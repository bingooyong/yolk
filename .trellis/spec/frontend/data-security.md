...
The shipped game uses browser-local authority. Progression, coins, skins, XP, and unlocks live in localStorage; no current game call invokes `getSql`, `authMiddleware`, app-data connector tools, or a server function. A player can therefore alter local progression.

Do not add server-authoritative assumptions to this game without a new product task. The cosmetic/local model is acceptable only while skins and economy do not grant competitive advantage.
...
Both expose the same `Sql` interface through parameterized tagged templates and `.query()`. PGLite pools, instances, and migration chains live on `globalThis` so Vite HMR does not create duplicate pools or races.
...
- `__Host-` cookie naming with Secure/Path/no-Domain constraints;
...
No `/api/rtc` route and no current game instantiation were found. Treat this as dormant library code, not active multiplayer support. Do not assume matchmaking, persistence, or server authority exists.
